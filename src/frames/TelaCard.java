...
public class TelaCard extends javax.swing.JFrame {  
    ...
    public TelaCard(Deck deck) {
        ...
        if(deck.isCarregouDados() == false) {
            ...
            // evita que os cards sejam duplicados
    ...
    private void adicionaCardsALista() {
    ...
    private void removeCardsDaLista() {
        if (deck.getCards().size() > 0) {
            Card[] cards = deck.getCards().toArray(new Card[0]);
            for (Card c : cards) {
                mod.removeElement(c.getFrente());
            }
        }
    }
    ...
    private void initEstudo(Deck deck) {
        ...
        timer = new javax.swing.Timer(1, (java.awt.event.ActionEvent e) -> {
            ...
            if (p.getX() > - panelFrente.getWidth()) {
                ...
                else if (deck.getCards().size() - idxAtual == 0) {  // fim do estudo
                    // mostrar painel de pontuacao
                    panelEstudo.setVisible(false);
                    panelScore.setVisible(true);
                    // atualiza deck de acordo com os acertos
                    deck.setCards(estudo.novaOrdemCards());
                    removeCardsDaLista();
                    adicionaCardsALista();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> {
            new TelaCard(null).setVisible(true);
        });
    }
